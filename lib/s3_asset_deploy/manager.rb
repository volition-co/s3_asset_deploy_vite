...
class S3AssetDeploy::Manager
  ...
  # Some assets generated by vite are dynamic and don't belong to a file. You
  # can mark them and pass in the flag you use to mark them, and we'll only keep
  # the latest version of those.
  def clean(version_limit: 2, version_ttl: 3600, removed_ttl: 172800, dry_run: false, dynamic_flag: nil)
    verify_no_duplicate_assets!
    ...
    remote_asset_collector.grouped_assets.each do |original_path, versions|
      ...
      versions_to_delete = versions_to_delete.each_with_index.drop_while do |version, index|
        if !current_asset
          ...
          asset_version_limit = original_path.include?(dynamic_flag) ? 0 : version_limit
          # Keep if under age or within the version_limit
          version_age = [0, Time.now - version.last_modified].max
          drop = version_age < version_ttl || index < asset_version_limit
